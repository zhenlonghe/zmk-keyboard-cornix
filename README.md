...
- **Soft-off with single-key wake** — a cross-half combo (base layer) powers both halves down into System OFF; a dedicated wake key on each half brings it back. With the indicator shield, each half confirms with a white double-blink right before cutting power — a half that doesn't blink didn't receive the off command. Wake columns deliberately avoid the combo columns so a still-held combo key cannot immediately re-wake the board. Note: soft-off/combo is split-direct only for now — the dongle build does not yet define it.
