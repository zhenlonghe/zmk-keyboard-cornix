...
- **RGB LED 指示扩展板**（`cornix_indicator`）用于电池 / 连接状态（自动熄灭并切断 LED 供电，额外功耗很小）。
- **可靠性与自恢复**（详见下文）：
  - BLE 双断链恢复兜底 —— 左右半与主机链路同时卡死时，等待 30 秒后恢复，仅在断链时用户近期有按键活动才触发。
  - 固件停机自恢复 —— fatal 错误改为冷重启，并有 30 秒硬件看门狗与崩溃循环保护。
...
- **`cornix_indicator`**: 启用 RGB LED 指示灯以显示电池状态和连接状态的扩展板。空闲时会切断 LED 供电，额外功耗不大。
...
- [x] 48 键布局 + 2 个编码器，自 v2.0
- [x] ec11 编码器，自 v2.2
- [x] 无 SD 镜像，自 v2.3
- [x] 支持各种适配器
- [x] 升级到 zephyr4.1 和 lvgl9，自 v2.7（适配器屏幕仍禁用）
- [x] RGB 状态指示（电池 / 充电 / 连接 / 大写锁定）
- [x] BLE 双断链快速恢复兜底
...
- **BLE 双断链兜底** —— 当左右分体链路与左半↔主机链路同时断开、且 ZMK 原生重连未能及时恢复时，左半等待 30 秒后执行一次冷重启以重新初始化 BLE 控制器。它仅在两条链路都曾连接后才武装，任一链路恢复即取消，并要求断链前 60 秒内有按键活动（闲置断链保持武装，用户回来在左半按下首键即调度）；USB 工作或睡眠时不会触发。
- **固件停机自恢复** —— Zephyr 默认 fatal 处理是永久死机；本项目将其覆盖为 `LOG_PANIC()` + 冷重启。另有 30 秒的 nRF52840 硬件看门狗（由指示灯工作循环喂狗），捕获不经过 fatal 路径的死锁和中断风暴。fatal、看门狗和 CPU lockup 共用 retained 崩溃计数：连续 3 次失败后进入 bootloader，不再无限重启（早到无法进行该检查的崩溃在第 4 次直接断电）；稳定运行 60 秒后自动清零。两侧均生效。
- **软关机 + 单键唤醒** —— 基础层的跨半 combo 将两侧关机进入 System OFF；每半有专用唤醒键将其唤醒。启用指示灯扩展板时，每半断电前会白色双闪确认——哪一半没闪，说明它没收到关机命令。唤醒列刻意避开 combo 所在列，避免仍被按住的 combo 键在关机瞬间立即重新唤醒。注意：软关机 / combo 目前仅分体直连支持，适配器构建尚未定义。

### RGB 状态指示

Cornix 每半有 2 颗可寻址 LED——**内侧**与**外侧**各一颗。`cornix_indicator` 扩展板将其用作状态指示灯，复现原始 RMK 固件的表意。在每半的构建目标中加入 `cornix_indicator` 扩展板即可启用。

所有指示均平滑淡入淡出，连接类动画最多 60 秒后自动熄灭。两颗 LED 全灭时扩展板会切断 LED 供电轨，因此键盘进入稳定状态后 LED 几乎全程断电；深睡与软关机时供电轨强制关闭（软关机断电前两颗灯会先白色双闪，作为关机确认）。相比无 LED 构建仍有少量额外功耗（状态轮询、短暂动画），但不存在 LED 常亮耗电。

**内侧 LED——电池与充电（两半相同），左半额外承担分体链路指示。** 优先级：充电 > 低电量 > 分体链路。

| 状态 | 灯效 |
| --- | --- |
| 充电中（<95%） | 绿色呼吸 |
| 充满（≥95%，仍插电） | 绿色常亮 3 秒后熄灭 |
| 低电量（≤20% 且未充电） | 珊瑚色双闪 5 秒，每 5 分钟提醒一次 |
| 右半失联（仅左半） | 蓝色呼吸，最长 60 秒后熄灭 |
| 右半（重新）连上（仅左半） | 蓝色常亮 3 秒后熄灭 |

**外侧 LED——左半（central），面向主机。** 3 秒连接提示优先于广播动画；两者都不活跃时显示大写锁定。

| 状态 | 灯效 |
| --- | --- |
| 连上主机 / 切换 profile | 对应 profile 颜色常亮 3 秒后熄灭 |
| 广播中（profile 未连接） | profile 颜色呼吸，最长 60 秒后熄灭 |
| Caps Lock 开启 | 琥珀色常亮（开启期间持续） |

Profile 0–4 颜色依次为：紫、绿、蓝、红、粉。

**外侧 LED——右半（peripheral）。**

| 状态 | 灯效 |
| --- | --- |
| 已连上左半 | 蓝色常亮 3 秒后熄灭 |
| 寻找左半中 | 蓝色呼吸，最长 60 秒后熄灭 |

两半的内侧 LED 各自显示本半电池状态。
...
> 3. 添加 "cornix_indicator" 扩展板以启用 RGB led 灯。动画会自动超时熄灭、空闲时切断 LED 供电，额外功耗不大。
