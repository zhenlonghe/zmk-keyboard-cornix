...
Cornix 每半有 2 颗可寻址 LED。`cornix_indicator` 扩展板将其用作状态指示灯，复现原始 RMK 固件的效果：电池电量、充电、低电量告警、BLE profile、分体连接和大写锁定状态。在每半的构建目标中加入 `cornix_indicator` 扩展板即可启用。它会保持 LED 供电，因此比默认无 LED 构建功耗明显更高。
