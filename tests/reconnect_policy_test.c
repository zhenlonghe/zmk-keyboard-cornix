...
static void test_startup_does_not_schedule(void) {
    ...
    assert(cornix_reconnect_policy_update(&policy, false, false, false, false) ==
...
static void test_single_link_loss_uses_native_reconnect(void) {
    struct cornix_reconnect_policy policy = {0};
    assert(cornix_reconnect_policy_update(&policy, true, true, false, false) ==
           CORNIX_RECONNECT_NONE);
    assert(policy.armed);
    assert(cornix_reconnect_policy_update(&policy, true, false, false, false) ==
           CORNIX_RECONNECT_NONE);
...
static void test_dual_loss_schedules_recovery(void) {
    struct cornix_reconnect_policy policy = {0};
    cornix_reconnect_policy_update(&policy, true, true, false, false);
    assert(cornix_reconnect_policy_update(&policy, false, false, false, false) ==
...
static void test_reconnect_cancels_recovery(void) {
    struct cornix_reconnect_policy policy = {0};
    cornix_reconnect_policy_update(&policy, true, true, false, false);
    cornix_reconnect_policy_update(&policy, false, false, false, false);
    assert(cornix_reconnect_policy_update(&policy, true, false, false, false) ==
...
static void test_expiry_recovers_only_once(void) {
    ...
    cornix_reconnect_policy_update(&policy, true, true, false, false);
    cornix_reconnect_policy_update(&policy, false, false, false, false);
    assert(cornix_reconnect_policy_expired(&policy, false, false, false, false));
    assert(!policy.armed);
    assert(!policy.pending);
    assert(!cornix_reconnect_policy_expired(&policy, false, false, false, false));
}
...
static void test_sleep_cancels_recovery(void) {
    ...
    cornix_reconnect_policy_update(&policy, true, true, false, false);
    cornix_reconnect_policy_update(&policy, false, false, false, false);
    assert(cornix_reconnect_policy_update(&policy, false, false, false, true) ==
           CORNIX_RECONNECT_CANCEL);
    assert(!cornix_reconnect_policy_expired(&policy, false, false, false, true));
}

static void test_usb_active_does_not_schedule(void) {
    struct cornix_reconnect_policy policy = {0};
    cornix_reconnect_policy_update(&policy, true, true, false, false);
    assert(cornix_reconnect_policy_update(&policy, false, false, true, false) ==
           CORNIX_RECONNECT_NONE);
    assert(!policy.pending);
    assert(policy.armed);
}
...
static void test_usb_active_blocks_expiry(void) {
    ...
    cornix_reconnect_policy_update(&policy, false, false, false, false);
    assert(policy.pending);
    assert(!cornix_reconnect_policy_expired(&policy, false, false, true, false));
    assert(!policy.pending);
}
